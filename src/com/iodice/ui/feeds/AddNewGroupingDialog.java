...
public class AddNewGroupingDialog {
	...
	public static AlertDialog.Builder getAddDialog(List<String> selectedUrls, 
													Context context, 
													SelectorRefreshCallback selectorRefreshInterface) {		
		...
		builder = setupButtons(builder, view, selectedUrls, selectorRefreshInterface);
	...
	private static AlertDialog.Builder setupButtons(AlertDialog.Builder builder, 
			...
			final SelectorRefreshCallback selectorRefreshInterface) {
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if (userInput != null && !userInput.equals("")) {
					...
					for (int i = 0; i < size; i++) {
						formattedList.add(Text.toFirstLetterUppercase(userInputList.get(i)));
					}
					categories.addAll(formattedList);
				}
		    	...
		    	selectorRefreshInterface.refreshCurrentSelector();
			}
		});
