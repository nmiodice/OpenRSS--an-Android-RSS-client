...
public class TopicsActivity extends NavigationDrawerWithSpinner implements ListRefreshCallback {
    ...
    protected void onSaveInstanceState (Bundle outState) {
    	...
    	if (v != null) {
	    	if (v.getVisibility() == View.VISIBLE)
	    		outState.putBoolean(TopicsActivity.SEARCH_KEY, true);
	    	if (txt != null)
	    		outState.putString(TopicsActivity.SEARCH_TEXT_KEY, txt.getText().toString());
    	}
    ...
    protected void onRestoreInstanceState (Bundle savedInstanceState) {
    	if (savedInstanceState != null) {
    		View v = this.findViewById(R.id.article_search_box_container);
    		if (v != null) {
	    		boolean wasSearchVisible = savedInstanceState.getBoolean(TopicsActivity.SEARCH_KEY);
    		...
    		if (txt != null) {
	    		String oldSearchTerm = savedInstanceState.getString(TopicsActivity.SEARCH_TEXT_KEY);
	...
	private void displayArticleList(List<String> urlList) {
		...
		if (fMan.findFragmentByTag(TopicsActivity.LIST) == null) {			
			ArticleList list = new ArticleList();
			list.setFeeds(urlList);
			fTrans = fMan.beginTransaction();
			fTrans.add(R.id.rss_fragment_container, list, TopicsActivity.LIST);
			fTrans.commit();
		}
	}
	...
	public void handleCallbackEvent(int n, Object obj) {
		switch (n) {
			case TopicsActivity.CALLBACK_REDRAW_WITH_CACHED_DATA:
				this.redrawActiveArticleListWithCachedData();
				this.setupSearchBar();
				return;
			case TopicsActivity.CALLBACK_UPDATE_WITH_WEB_QUERY:
				this.updateCurrentListWithWebQuery();
				return;
			default:
